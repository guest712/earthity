...
function parseProfileTabParam(
  raw: string | string[] | undefined
): 'profile' | 'stats' | 'achievements' | 'settings' | undefined {
  const v = typeof raw === 'string' ? raw : Array.isArray(raw) ? raw[0] : undefined;
  if (v === 'profile' || v === 'stats' || v === 'achievements' || v === 'settings') return v;
  return undefined;
}

type ProfileSegmentTabProps = {
  active: boolean;
  onPress: () => void;
  emoji: string;
  title: string;
...
function ProfileSegmentTab({ active, onPress, emoji, title, description }: ProfileSegmentTabProps) {
  return (
    <TouchableOpacity
    ...
    >
      ...
      <Text style={[styles.tabDesc, active && styles.tabDescActive]} numberOfLines={2}>
        {description}
      </Text>
    </TouchableOpacity>
  );
  ...
  return (
    <View style={styles.fullStatsContainer}>
      ...
      <TouchableOpacity style={styles.signOutBtn} onPress={() => void signOut()}>
        <Text style={styles.signOutLabel}>{t.authSignOut}</Text>
      </TouchableOpacity>
    </View>
  );
}
...
export default function ProfileScreen() {
  ...
  const params = useLocalSearchParams<{ profileTab?: string | string[] }>();
  ...
  useLayoutEffect(() => {
    const next = parseProfileTabParam(params.profileTab);
    if (next !== undefined && params.profileTab !== undefined) {
      setActiveTab(next);
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <ProfileSegmentTab
          ...
          description={t.profileTabProfileDesc}
        />
        <ProfileSegmentTab
          active={activeTab === 'stats'}
          onPress={() => setActiveTab('stats')}
          emoji="📊"
          title={t.profileTabStats}
          description={t.profileTabStatsDesc}
        />
        <ProfileSegmentTab
          ...
          emoji="🏆"
          title={t.tabAwards}
          description={t.profileTabAchievementsDesc}
        />
        <ProfileSegmentTab
          active={activeTab === 'settings'}
          onPress={() => setActiveTab('settings')}
          emoji="⚙️"
          title={t.profileTabSettings}
          description={t.profileTabSettingsDesc}
        />
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  tabTitle: { fontSize: 13, color: 'rgba(255,255,255,0.55)', fontWeight: '600', textAlign: 'center', width: '100%' },
  tabTitleActive: { color: '#5aad6a' },
  tabDesc: {
    marginTop: 6,
    fontSize: 10,
    lineHeight: 13,
  ...
  tabDescActive: { color: 'rgba(255,255,255,0.52)' },
  ...
  signOutLabel: { fontSize: 15, fontWeight: '600', color: '#d08080', textAlign: 'center' },
  sectionTitle: { fontSize: 20, color: '#e8e4d8', textAlign: 'center', marginBottom: 16, letterSpacing: 0.5 },
  panelSection: { width: '100%', paddingTop: 8, alignSelf: 'stretch' },
});